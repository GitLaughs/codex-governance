...
终端 viewer 依赖仓库根目录 `package.json` 中声明的 `@xterm/xterm`。首次使用 `dashboard.html` 前先在仓库根目录执行：

```powershell
npm install
```

`dashboard.html` 直接加载 `../../node_modules/@xterm/xterm/...`；未安装依赖时，终端区只会退化为普通会话列表/占位提示。

## 核心能力

- `codex_governance.py`：读取 Git 工作区，按路径规则生成人工可读或 JSON 治理报告。
- `codex_launcher.py`：提供本地 HTTP API，负责中书省会话、部门会话、并发队列、结果归档、计划确认。
- `dashboard.html`：前端看板，展示报告、会话、分派方案、终端 viewer 和回传结果。
...
首次使用终端 viewer 前，先在仓库根目录安装前端依赖：

```powershell
npm install
```

`dashboard.html` 通过根目录 `package.json` 中的 `@xterm/xterm` 渲染终端区域；未安装依赖时会保留普通会话信息展示。
...
- 确认中书省回传的部门分派方案
- 查看会话、队列、回传结果
- 通过 `/api/session_stream` 渲染会话终端 viewer
- 通过 `/api/session_input` 向运行中的会话发送单行输入

终端交互仍保留本地 trusted launcher 边界：

- `dashboard.html` 只连默认本机 `127.0.0.1` launcher
- `/api/session_stream` 提供 launcher 维护的会话元数据，并对 launcher 启动的会话追加 Codex stdout/stderr 日志 tail；它不是远程 PTY
- `/api/session_input` 只把输入转发到本地已登记的 Codex 窗口，不开放任意命令执行
- 同屏方案是“1 个可切换交互 viewer + 最多 3 个运行会话概览卡片”，不是多路并行可输入终端
- `/api/session_stream` 由 launcher 每秒检查一次，仅在转录内容变化时推送新事件

### 4. 终端执行层

`run_codex_prompt.py` 把 prompt 写入文件后再启动 Codex CLI，解决 Windows 控制台 UTF-8 和长 prompt 传递问题。launcher 会同时把 Codex stdout/stderr 镜像到 `.tmp/codex_governance_logs/`，供 dashboard 的终端 viewer 和运行会话卡片实时读取最近输出。
...
- `/api/session_stream?id=<session_id>`：SSE 终端 viewer 数据流
