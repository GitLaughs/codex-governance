...
- `../codex_terminal/`：xterm.js + node-pty + WebSocket 的本地浏览器终端原型，可由看板按需 iframe 嵌入。
...
- 确认中书省回传的部门分派方案
- 查看会话、队列、回传结果
- 按需启动一个本机浏览器终端 iframe，或为治理会话打开 transcript 预览；launcher 只分发 tokenized loopback URL，PTY 逻辑仍留在 `tools/codex_terminal/`
...
- `/api/browser_terminals`：读取浏览器终端 sidecar 状态
- `/api/report?mode=worktree|staged&base=<git_ref>`：返回治理报告
- `/api/browser_terminals`：读取本机终端 / 会话预览 sidecar 状态
...
- `/api/browser_terminal/start`
- `/api/browser_terminal/close`
- `/api/session_browser_terminal/start`
...
- launcher 只用 `127.0.0.1` + 随机端口启动 `tools/codex_terminal/server.js`
- `tools/codex_terminal` 负责运行时 token 和 WebSocket 鉴权
- 看板仅 iframe 嵌入返回 URL，不直接实现 PTY 或绕过 token
